...
const BattleMap = ({
    ...
    if (!selectedRow || attackSelection.current != selectedRow.name) {
      ...
      if (!players[userName]?.battleMode?.actionAttempted && players[userName]?.battleMode?.usersTargeted?.length > 0) {
        setPlayers((prevPlayers) => ({
          ...prevPlayers,
          [userName]: {
            // userName is the name/key of the user you want to update
  ...
  useEffect(() => {
    if (circleStop && players[userName].battleMode.yourTurn) {
      ...
      coveredPlayers.forEach((figure) => {
        const attacksArray = Object.values(players[userName].attacks);
        const attackData = attacksArray.filter((attack) => attack.name === selectedRow?.name);
        console.log("attackData: ", attackData);

        if ((attackData[0].type == "spell" || attackData[0].type == "melee") && figure.type == "enemy") {
  ...
  const handleMapClick = (e) => {
    ...
    if (
      ...
      clickedGridX == players[userName].pingXPosition &&
      clickedGridY == players[userName].pingYPosition
    ) {
  ...
  useEffect(() => {
    ...
    if (players[userName]?.battleMode?.yourTurn && selectedRow && !players[userName]?.battleMode?.actionAttempted) {
      const attacksArray = Object.values(players[userName].attacks);
      const attack = attacksArray.filter((attack) => attack.name === selectedRow?.name);
      console.log("attack", attack);
      if (attack) {
        setAttackDistance(attack[0].distance * travelZoneRadius + 3); // +3 for margin
        // Assuming xWidth and yWidth are diameters, calculating average radius
        const radius = (attack[0].xWidth + attack[0].yWidth) / 4; // Divided by 4 because we need the average radius
  ...
  useEffect(() => {
    ...
    Object.entries(players).forEach(([playerName, playerData]) => {
      ...
      if (
        ...
        prevPlayersBattleData.current[playerName]?.battleMode?.enemyAttackAttempt == "INIT"
      ) {
        ...
        playerData?.battleMode?.enemyAttackAttempt == "FAIL" &&
        prevPlayersBattleData.current[playerName]?.battleMode?.enemyAttackAttempt == "INIT"
      ) {
        setShowEnemyResult((prevState) => ({
          ...prevState,
      ...
      if (
        playerData.battleMode.damageDelt > 0 &&
        (prevPlayersBattleData.current[playerName]?.battleMode.damageDelt < 1 || !prevPlayersBattleData.current[playerName]?.battleMode.damageDelt)
      ) {
        ...
        const attacksArray = Object.values(playerData.attacks);
        const attackData = attacksArray.filter((attack) => attack.name === playerData.battleMode.attackUsed);

        if (attackData[0].type == "spell") {
  ...
  const getCoveredCells = (circleCenter, radius, gridSpacing) => {
    ...
    for (let i = leftIndex; i <= rightIndex; i++) {
      for (let j = topIndex; j <= bottomIndex; j++) {
        ...
        const distanceToClosestPoint = Math.sqrt((closestPoint.x - circleCenter.x) ** 2 + (closestPoint.y - circleCenter.y) ** 2);
        ...
        if (distanceToClosestPoint < radius) {
          ...
          const distanceToCellCenter = Math.sqrt((cellCenter.x - circleCenter.x) ** 2 + (cellCenter.y - circleCenter.y) ** 2);
    ...
    <div
      ...
      {imageLoaded && (
        <Stage
          ...
          <Layer>
            <Image image={image} scaleX={scale} scaleY={scale} className={animationClass} />
            {drawGrid()}
            <Circle
            ...
            {Object.entries(players).map(([playerName, playerData]) => (
              <React.Fragment key={playerName}>
                <>
                  {playerData?.pingXPosition && <FlickeringRect playerData={playerData} gridSpacing={gridSpacing} />}
